...
const toLastNames = people => {
  // Solution code here...
  let userFullnames = people.map((e)=>{
...
const addValues = (arr) => {
  // Solution code here...
  const addingNumber = arr.reduce((acc, number)=>{
    acc+=number;
    return acc;
  },0);
  return addingNumber;
...
const addPurchases = (arr) => {
  // Solution code here...
  const purchasedItem = arr.reduce((acc, item) =>{
    acc[item.purchasePrice]+= item.purchasePrice;
    return acc;
  },0);
